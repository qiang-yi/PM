...
from pointpats import PointPattern as PP
import warnings
...
class PoissonClusterPointProcess(PointProcess):
    ...
    def realize(self, n):
        ...
        # if want to keep all nodes, then quote the following line
        #cents = self.window.filter_contained(cents)
        self.parentnodes = PP(cents, window=self.window)
